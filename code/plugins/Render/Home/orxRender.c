...
static orxINLINE void orxRender_Home_RenderProfiler()
{
      ...
      s32MarkerID = orxProfiler_GetNextSortedMarkerID(s32MarkerID))
  {
    ...
    && (orxProfiler_GetMarkerPushCount(s32MarkerID) > 0))
    {
      ...
      orxDisplay_TransformText(acLabel, NULL, 0, pstFontBitmap, orxFont_GetMap(pstFont), &stTransform, orxDISPLAY_SMOOTHING_NONE, orxDISPLAY_BLEND_MODE_ALPHA);
  ...
  && (orxStructure_TestFlags(pstGraphic, orxGRAPHIC_KU32_FLAG_2D | orxGRAPHIC_KU32_FLAG_TEXT)))
  {
    ...
    if((orxEvent_Send(&stEvent) != orxSTATUS_FAILURE) && (pstBitmap != orxNULL))
    {
      ...
      if((stPayload.stObject.pstTransform->fScaleX != orxFLOAT_0) && (stPayload.stObject.pstTransform->fScaleY != orxFLOAT_0))
      {
        ...
        {
          /* Transforms text */
          eResult = orxDisplay_TransformText(orxText_GetString(pstText), orxText_GetMarkerArray(pstText), orxText_GetMarkerCount(pstText), pstBitmap, orxFont_GetMap(pstFont), stPayload.stObject.pstTransform, _eSmoothing, _eBlendMode);
        }
      }
      else
      {
